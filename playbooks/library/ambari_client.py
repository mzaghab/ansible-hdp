...
# create cluster : https://github.com/apache/ambari/blob/trunk/ambari-client/python-client/src/examples/create.py
...
class AmbariApi:
  ...
  def __init__(self, server_host, server_port=8080,
    ...
    @param server_host: The hostname of the Ambari server.
    @param server_port: The port of the server. Defaults to 8080.
    @param username: Login name, Default to admin.
    @param password: Login password, Default to admin.
    @param version: API version.
    @return: Resource object.
    """
    protocol = "http" # TODO add use of use tls
    self._version = version
    self._auth=(username, password)
    self._base_url = "%s://%s:%s/api/v%s" % (protocol, server_host, server_port, version)
...
def main():
    ...
    try:
        ambari_host = '192.168.161.101'
        ambari_port = '8080'
        ambari_username = 'admin'
        ambari_password = 'admin'
        cluster_name = "VAGRANT_CLUSTER"
        hosts = ['vag.hdp101.dcn.fr.carrefour.com','vag.hdp102.dcn.fr.carrefour.com',
                            'vag.hdp103.dcn.fr.carrefour.com','vag.hdp104.dcn.fr.carrefour.com']
        #        services = "zookeeper", "yarn", "oozie", "hive", "hue", "impala", "spark_on_yarn"]

        services = [{'name':'hdfs', 'type':'HDFS'},
        ...
        ambari = AmbariApi(ambari_host, ambari_port, ambari_username, ambari_password, version=1)
#        ambari.
#        cluster = client.create_cluster(cluster_name, "HDP-1.3.0")
#        print cluster
# cluster list
# curl -u admin:admin http://192.168.162.101:8080/api/v1/clusters
# hosts of registered list
# curl -u admin:admin http://192.168.162.101:8080/api/v1/hosts
# liste of services
# curl -u admin:admin http://192.168.162.101:8080/api/v1/services
# liste of cluster services
# curl -u admin:admin http://192.168.162.101:8080/api/v1/clusters/test/services
#  create cluster
# curl -X POST -u admin:admin -H "X-Requested-By: ambari" -d '{"Clusters": {"version" : "HDP-2.6"}}' http://192.168.162.101:8080/api/v1/clusters/test
# delete cluster
# curl -X DELETE -u admin:admin -H "X-Requested-By: ambari" -d '{"Clusters": {"version" : "HDP-2.6"}}' http://192.168.162.101:8080/api/v1/clusters/test
# add hosts to cluster
# curl -X POST -u admin:admin -H "X-Requested-By: ambari" http://192.168.162.101:8080/api/v1/clusters/TT/hosts/vag-hdp001.mzaghab.com
# add service to cluster
# curl -X POST -u admin:admin -H "X-Requested-By: ambari" http://192.168.162.101:8080/api/v1/clusters/TT/services/HDFS
# update repo url
# # curl -X PUT -u admin:admin -H "X-Requested-By: ambari" http://192.168.162.101:8080/api/v1/stacks/HDP/versions/2.6/repository_versions/1
#  add services
